...
class CityMap:
    ...
    def search(self, start, goal):
        ...
        # BLList.append(State(start))
        # prev_state = BLList[0]
        while len(node_list) > 0:
            # print(node_list)
            state = node_list[0] 
            code = node_list[0].stop
            # node = self.stops[node_code]
            # node_list.remove(node_code)
            del node_list[0]
            # node_state = State(node_code, prev_state)
            if code not in visited:
                visited.add(code)
                if code == goal:
                    return state
                for neighbor in self.get_neighbors_codes(code):
                    node_list.append(State(neighbor, state))
        return None