...
def load_data(source_file):
    ...
    try:
        ...
        with open(os.path.join(dir_path, '..', source_file), encoding='utf-8') as file:
...
class State:
    ...
    def heuristic(self):
        ...
        """
        # Implement me
        return dist((self.goal['x'], self.goal['y']), (self.stop['x'], self.stop['y'])) / MAX_TRAM_SPEED
...
class CityMap:
    ...
    def search(self, start, goal, time_of_beginning):
        ...
        # Implement me
        # print(to_visit.get().get_stop())
        while not to_visit.empty():
            node : State = to_visit.get()
            # print(node)
            if node.get_stop_code() == goal['code']:
                return node
            for neighbor_code in self.get_neighbors_codes(node.get_stop_code()):
                neighbor = self.stops[neighbor_code]
                to_visit.put(State(neighbor, node.get_time() + self.fastest_transition(node.get_stop_code(), neighbor_code, node.get_time()), node))
            # visited.append(node)
        return None
