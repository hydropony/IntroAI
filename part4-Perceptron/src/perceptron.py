...
class Perceptron:
    ...
    def train(self, target_char, opposite_char, steps):
        # Implement method
        self.weights = [0] * (28 * 28)
        self.weights = np.array(self.weights)
        training_data = self.data[5000:]

        training_data = [d for d in training_data if d['char'] in (target_char, opposite_char)]
        
        for _ in range(steps):
            d = training_data[random.randint(0, len(training_data) - 1)]
            (x, y) = (d['vector'], d['char'])
            x = np.array(x)
            z = np.dot(x, self.weights)
            if z <= 0 and y == target_char:
                self.weights = np.add(self.weights, x)
            elif z > 0 and y == opposite_char:
                self.weights = np.add(self.weights, -x)
        return self.weights
